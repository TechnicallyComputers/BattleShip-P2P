...
void mnVSModeMakeVSStartButton()
{
    ...
    mnVSModeUpdateButton(button_gobj, (sMNVSModeCursorIndex == nMNVSModeOptionStart) ? nMNOptionTabStatusHighlight : nMNOptionTabStatusNot);

    button_sobj = lbCommonMakeSObjForGObj(button_gobj, lbRelocGetFileData(Sprite*, sMNVSModeFiles[1], llMNVSModeVSStartTextSprite));
    button_sobj->sprite.attr &= ~SP_FASTCOPY;
    button_sobj->sprite.attr |= SP_TRANSPARENT;
    if (mnPortTryApplyVsSubmenuLabelPng(button_sobj, "offline_vs.png", 120.0F, 31.0F) == FALSE)
    {
...
void mnVSModeMakeRuleButton()
{
    ...
    button_sobj = lbCommonMakeSObjForGObj(rule_button_gobj, lbRelocGetFileData(Sprite*, sMNVSModeFiles[1], llMNVSModeVSStartTextSprite));
    button_sobj->sprite.attr &= ~SP_FASTCOPY;
    button_sobj->sprite.attr |= SP_TRANSPARENT;
    if (mnPortTryApplyVsSubmenuLabelPng(button_sobj, "online_vs.png", 97.0F, 70.0F) == FALSE)
    {
        button_sobj->sprite.red = 0x00;
        button_sobj->sprite.green = 0x00;
        button_sobj->sprite.blue = 0x00;
...
void mnVSModeMakeTimeStockButton()
{
    ...
    time_stock_button_sobj = lbCommonMakeSObjForGObj(time_stock_button_gobj, lbRelocGetFileData(Sprite*, sMNVSModeFiles[1], llMNVSModeVSStartTextSprite));
    time_stock_button_sobj->sprite.attr &= ~SP_FASTCOPY;
    time_stock_button_sobj->sprite.attr |= SP_TRANSPARENT;
    if (mnPortTryApplyVsSubmenuLabelPng(time_stock_button_sobj, "replays.png", 74.0F, 109.0F) == FALSE)
    {
...
void mnVSModeMakeVSOptionsButton(void)
{
    ...
    gcAddGObjDisplay(button_gobj, lbCommonDrawSObjAttr, 2, GOBJ_PRIORITY_DEFAULT, ~0);
    mnVSModeMakeButton(button_gobj, 120.0F, 148.0F, 17);
...
{
    GObj** buttons[/* */] = { &sMNVSModeButtonGObjVSStart, &sMNVSModeButtonGObjRule, &sMNVSModeButtonGObjTimeStock };
        ...
        if (scSubsysControllerGetPlayerTapButtons(A_BUTTON | START_BUTTON) != FALSE)
        {
            switch (sMNVSModeCursorIndex)
            {
                case nMNVSModeOptionStart:
                    ...
                    gSCManagerSceneData.scene_prev = gSCManagerSceneData.scene_curr;
                    gSCManagerSceneData.scene_curr = nSCKindVSOfflineClassic;

                    return;

                case nMNVSModeOptionRule:
                    func_800269C0_275C0(nSYAudioFGMMenuSelect);
                    mnVSModeUpdateButton(sMNVSModeButtonGObjRule, nMNOptionTabStatusSelected);
                    mnVSModeSaveSettings();
